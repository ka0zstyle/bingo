...
currencyDisplayBtn?.addEventListener('click', async ()=>{
  ...
  try {
    // usar save_settings o el alias agregado save_currency_settings
...
/* ========== Nueva Moneda (modal) ========== */
function openNewCurrencyModal(){ openOverlay('newCurrencyModal'); setTimeout(()=>document.getElementById('new-currency-name').focus(), 50); }
function closeNewCurrencyModal(){ closeOverlay('newCurrencyModal'); }

document.getElementById('new-currency-save')?.addEventListener('click', async ()=>{
  ...
  const rate   = document.getElementById('new-currency-rate').value.trim();
  const isDef  = document.getElementById('new-currency-default').checked ? '1' : '0';

  if (!name || !code || !symbol || !rate || isNaN(rate)) {
    return notify('Completa nombre, código, símbolo y una tasa válida.', 'error');
  }
  ...
  const btn = document.getElementById('new-currency-save');
  const original = btn.textContent; btn.textContent = 'Creando...'; btn.disabled = true;
  try {
    await postForm('api.php?action=create_currency', fd);
    notify('Moneda creada.');
    setTimeout(()=> window.location.reload(), 600);
  } catch(err){ notify(err.message, 'error'); }
...
document.getElementById('save_settings')?.addEventListener('click', async ()=>{
  const fd = new FormData();
  // Nombres “cortos” que la API mapea a claves carton_*
  fd.append('header_color',      document.getElementById('header_color').value);
  fd.append('header_text_color', document.getElementById('header_text_color').value);
  fd.append('free_cell_color',   document.getElementById('free_cell_color').value);
  fd.append('free_text_color',   document.getElementById('free_text_color').value);
  fd.append('number_color',      document.getElementById('number_color').value);
  fd.append('border_color',      document.getElementById('border_color').value);
  fd.append('font',              document.getElementById('font').value);
  fd.append('grid_style',        document.getElementById('grid_style').value);
  fd.append('center_content',    document.getElementById('center_content').value);
  fd.append('center_is_free',    document.getElementById('center_is_free').checked ? '1' : '0');
