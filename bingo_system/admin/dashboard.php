...
$sqlLast = "SELECT p.id, p.owner_name, p.owner_email, p.total_local, p.total_usd, p.status, p.created_at, e.name as event_name
          ...
          <table class="table">
            ...
            <tbody>
              <?php if (empty($last_payments)): ?>
                <tr><td colspan="7" class="small-muted">Sin resultados.</td></tr>
              <?php else: foreach ($last_payments as $p): ?>
                <tr>
                  ...
                  <td><?= htmlspecialchars(date('d/m/Y H:i', strtotime((string)$p['created_at']))) ?></td>
                </tr>
