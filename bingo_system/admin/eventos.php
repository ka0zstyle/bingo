...
// Helper de moneda para conversión en tiempo real
...
if (file_exists(__DIR__ . '/../includes/currency_helper.php')) {
    ...
    $currency_helper_loaded = function_exists('get_currency_settings') && function_exists('local_to_usd') && function_exists('format_local') && function_exists('format_usd');
}
...
// Configuración de moneda por defecto (desde helper si está disponible)
if ($currency_helper_loaded) {
    $cur = get_currency_settings(); // ['code','symbol','rate','show_decimals']
    $local_currency_code = $cur['code'] ?: 'VES';
} else {
    // Fallback si el helper no está disponible por alguna razón
    ...
    <div class="event-list">
        ...
        <?php else: ?>
            <?php foreach ($events as $ev): ?>
                <?php 
                    $active = ((int)($ev['is_active'] ?? 0) === 1);
                    $priceLocal = (float)($ev['price_local'] ?? 0);
                    // Conversion en tiempo real a USD
                    if ($currency_helper_loaded) {
                        $priceUsd = local_to_usd($priceLocal);
                        $priceLocalFmt = format_local($priceLocal);
                        $priceUsdFmt   = format_usd($priceUsd);
                    } else {
                        // Fallback básico si no está el helper
                        $rateRow = $pdo->query("SELECT code, rate FROM bingo_currencies WHERE is_default = 1 LIMIT 1")->fetch(PDO::FETCH_ASSOC);
                        $code = (string)($rateRow['code'] ?? 'VES');
                        $rate = (float)($rateRow['rate'] ?? 1);
                        $priceUsd = ($code === 'USD') ? $priceLocal : ($priceLocal / max(1e-12, $rate));
                        $priceLocalFmt = number_format($priceLocal, 2, ',', '.'); // sin símbolo
                        $priceUsdFmt   = '$' . number_format($priceUsd, 2, ',', '.');
                    }
                ?>
                <div class="event-card" id="event-card-<?php e($ev['id']); ?>">
                    ...
                    <div class="event-details">
                        ...
                        <div class="detail-item"><strong>Precio (<?php e($local_currency_code); ?>):</strong> <?php e($priceLocalFmt); ?></div>
                        <div class="detail-item"><strong>Precio (USD):</strong> <?php e($priceUsdFmt); ?></div>
                    ...
                    <div class="card-footer">
                        <?php
                          $js = htmlspecialchars(json_encode([
                              ...
                              // 'price_usd' eliminado (se calcula en tiempo real)
