...
function updateStatus(purchaseId, newStatus, buttonEl) {
  ...
  if (!csrfMeta) { notify('Error: Falta el token CSRF en <meta>.', 'error'); return; }
  ...
  if (buttonEl) { buttonEl.disabled = true; buttonEl.textContent = 'Procesando...'; }
  ...
  .then(async (res) => {
    ...
    try {
      ...
      const plain = raw.replace(/<[^>]*>/g, ' ').trim().slice(0, 400);
      throw new Error(plain || `Respuesta no válida del servidor (HTTP ${res.status}).`);
    }
  })
  .then((data) => {
    if (data.success) {
      notify(data.message || 'Operación realizada.');
      // Quitar la tarjeta aprobada/rechazada si existe
      const card = document.getElementById(`payment-card-${purchaseId}`);
      if (card) { card.classList.add('removing'); setTimeout(() => card.remove(), 500); }
      if (data.stats) updateStats(data.stats);
    } else {
      notify(data.message || 'Ocurrió un error al procesar la operación.', 'error');
    }
  })
  .catch((err) => {
    console.error('Error en updateStatus:', err);
    notify(err.message || 'Error de red al procesar la solicitud.', 'error');
  })
  .finally(() => {
    if (buttonEl) { buttonEl.disabled = false; buttonEl.textContent = (newStatus === 'approved' ? 'Aprobar' : 'Rechazar'); }
  });
}
