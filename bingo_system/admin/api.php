...
/** Normaliza cualquier estructura de cartón a un arreglo por columnas B..O o grilla 5x5 compatible. */
function normalize_numbers($data) {
    ...
    // Fallback vacío con FREE al centro
    return [
        'B' => ['', '', '', '', ''],
        'I' => ['', '', '', '', ''],
        'N' => ['', '', 'FREE', '', ''],
...
// Autenticación básica del admin
...
// Verifica CSRF para todos los POST
...
switch ($action) {
    ...
    case 'save_event': {
        ...
        $stmt_rate = $pdo->query("SELECT rate FROM currencies WHERE is_default = 1 LIMIT 1");
        $rate = (float)($stmt_rate->fetchColumn() ?: 1);
        $price_usd = $price_local / $rate;

        if ($id > 0) {
            $stmt = $pdo->prepare("UPDATE bingo_events SET name = ?, event_date = ?, price_local = ?, price_usd = ? WHERE id = ?");
            $stmt->execute([$name, $date, $price_local, $price_usd, $id]);
        } else {
            $stmt = $pdo->prepare("INSERT INTO bingo_events (name, event_date, price_local, price_usd) VALUES (?, ?, ?, ?)");
            $stmt->execute([$name, $date, $price_local, $price_usd]);
        }
    ...
    case 'adjust_card_quantity': {
        ...
        if (!$eventId) {
            $pdo->rollBack(); json_err('La compra no existe.', 400);
        }

        $stmt = $pdo->prepare("SELECT price_local, price_usd FROM bingo_events WHERE id = ?");
        $stmt->execute([$eventId]);
        $eventPrices = $stmt->fetch(PDO::FETCH_ASSOC);
        if (!$eventPrices) {
            $pdo->rollBack(); json_err('El evento asociado no existe.', 400);
        }
        $pricePerCardLocal = (float)$eventPrices['price_local'];
        $pricePerCardUsd   = (float)$eventPrices['price_usd'];
        ...
        if ($quantityChange > 0) {
            ...
            for ($i = 0; $i < $quantityChange; $i++) {
                ...
                if ($code === '') {
                    $code = 'B-' . (1000 + time() % 100000) . '-' . chr(rand(65, 90));
                }
    ...
    case 'update_purchase_status': {
        ...
        if ($newStatus === 'approved') {
            ...
            // Recalcular y devolver los contadores actualizados
    ...
    case 'save_payment_method': {
        ...
        if ($id > 0) {
            $stmt = $pdo->prepare("UPDATE payment_methods SET name = ?, details = ? WHERE id = ?");
    ...
    case 'save_currency_settings': {
        ...
        $show_decimals = filter_var($payload['show_decimals'] ?? '1', FILTER_VALIDATE_INT);

        $stmt = $pdo->prepare("
            INSERT INTO bingo_settings (setting_key, setting_value)
            VALUES ('show_decimals', ?)
            ON DUPLICATE KEY UPDATE setting_value = VALUES(setting_value)
        ");
        $stmt->execute([ (string)$show_decimals ]);
        json_ok(['message' => 'Ajustes de moneda guardados.']);
    }

    case 'save_currency': {
        if ($current_role !== 'admin') json_err('Permisos insuficientes.', 403);
        $id = filter_input(INPUT_POST, 'id', FILTER_VALIDATE_INT);
        $rate_inverse = trim((string)($_POST['rate_inverse'] ?? ''));
        if (!$id || !is_numeric($rate_inverse) || (float)$rate_inverse <= 0) {
            json_err('ID o tasa de cambio inválida.', 400);
        }

        $rate = 1 / (float)$rate_inverse;
        $stmt = $pdo->prepare("UPDATE currencies SET rate = ? WHERE id = ?");
    ...
    case 'set_default_currency': {
        ...
        $pdo->beginTransaction();
        $pdo->prepare("UPDATE currencies SET is_default = 0")->execute();
        $stmt = $pdo->prepare("UPDATE currencies SET is_default = 1 WHERE id = ?");
        $stmt->execute([$id]);
        $pdo->commit();
