...
if (file_exists(__DIR__ . '/includes/currency_helper.php')) {
    ...
    $currency_settings = ['symbol' => 'Bs.', 'code' => 'VES', 'rate' => 197.5, 'show_decimals' => true];
}
...
// IMPORTANTE: ya no pedimos price_usd (fue eliminado). Solo price_local.
$stmt = $pdo->prepare("
    SELECT name, price_local 
    FROM bingo_events 
    WHERE id = ? AND is_active = 1 AND event_date > NOW()
");
...
$payment_methods_stmt = $pdo->query("SELECT * FROM bingo_payment_methods WHERE is_active = 1");
$payment_methods = $payment_methods_stmt->fetchAll(PDO::FETCH_ASSOC);

// Valor base para cobro en la moneda por defecto (local)
$price_local = (float)$evento['price_local'];

// Cálculo de USD en tiempo real (solo para mostrar, no para cobrar)
if (function_exists('local_to_usd')) {
    $price_usd = local_to_usd($price_local);
} else {
    // Fallback si no está el helper: USD = Local / rate (si default != USD)
    $price_usd = ($currency_settings['code'] === 'USD') 
        ? $price_local 
        : ($price_local / max(1e-12, (float)$currency_settings['rate']));
}

// Para JS: precio por cartón en la moneda local (el que se cobra)
$price_for_js = $price_local;
...
<div class="form-container">
    ...
    <form action="procesar_compra.php" method="POST" enctype="multipart/form-data">
        ...
        <input type="hidden" id="price_per_carton" value="<?php echo htmlspecialchars($price_for_js, ENT_QUOTES); ?>">
        <input type="hidden" id="rate_current" value="<?php echo htmlspecialchars((string)$currency_settings['rate'], ENT_QUOTES); ?>">
        <input type="hidden" id="default_code" value="<?php echo htmlspecialchars($currency_settings['code'], ENT_QUOTES); ?>">
        ...
        <div class="form-step">
            ...
            <div class="form-group total-amount-display">
                ...
                <div style="margin-top:6px; color:#64748b; font-weight:600;">
                    Aproximado en USD: <span id="total_amount_usd_display"></span>
                </div>
            </div>
                    ...
                    <?php foreach ($payment_methods as $method): ?>
                        <div class="payment-method-card">
                            ...
                            <input 
                                type="radio" 
                                name="payment_method" 
                                value="<?php echo htmlspecialchars($methodName); ?>" 
                                id="pm_<?php echo (int)$method['id']; ?>" 
                                required <?php if ($payment_method_val === $methodName) echo 'checked'; ?>
                            >
                            <label class="payment-method-header" for="pm_<?php echo (int)$method['id']; ?>">
                            ...
                            <div class="payment-details">
                                ...
                                if (count($pairs) > 0 && strpos($details, ':') !== false) {
                                    ...
                                    echo '<p>' . nl2br(htmlspecialchars($details)) . '</p>'; 
                                }
                                ?>
                            </div>
                        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const pricePerCartonLocal  = parseFloat(document.getElementById('price_per_carton').value);
    const totalAmountDisplay   = document.getElementById('total_amount_display');
    const totalUsdDisplay      = document.getElementById('total_amount_usd_display');

    const currencyConfig = <?php echo json_encode($currency_settings, JSON_UNESCAPED_UNICODE); ?>;
    // currencyConfig = { code, symbol, rate, show_decimals }
    ...
    function calculateAndDisplayTotal() {
        const qty = Math.max(0, parseInt(cantidadInput.value, 10) || 0);

        // Total en moneda local (la que se cobra)
        const totalLocal    = qty * pricePerCartonLocal;
        const localDecimals = currencyConfig.show_decimals ? 2 : 0;
        totalAmountDisplay.textContent = currencyConfig.symbol + ' ' + formatNumber(totalLocal, localDecimals);

        // Aproximado en USD (dinámico): 
        // Si default = USD, total USD = totalLocal
        // Si default != USD, USD = Local / rate (rate = cuántas unidades locales hay en 1 USD)
        let totalUsd;
        if (currencyConfig.code === 'USD') {
            totalUsd = totalLocal;
        } else {
            const rate = Math.max(1e-12, parseFloat(currencyConfig.rate || '0'));
            totalUsd = totalLocal / rate;
        }
        totalUsdDisplay.textContent = '$ ' + formatNumber(totalUsd, 2);
    }
